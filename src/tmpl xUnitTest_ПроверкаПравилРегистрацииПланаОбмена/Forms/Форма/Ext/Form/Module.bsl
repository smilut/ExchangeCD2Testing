...
|РегистрСведений.дмнПроцентРаспределенияРасходовПоНоменклатурнымГруппам,
|РегистрСведений.дмнЭталонныеСправочники,
|ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения,
|Справочник.НаборыДополнительныхРеквизитовИСведений,
|Справочник.дмнХарактеристикиВидовСХРабот,
|РегистрСведений.дмнСтатусыЗаказНарядов";
...
Функция ИсточникиДляПодписокРегистрацииИзменений()
	...
	соотв.Вставить(типыОбъектовОбмена.ПланВидовХарактеристик,	
		Метаданные.ПодпискиНаСобытия.дмнПередЗаписьюОбъектовДляОбменаБСХП_БСХП.Источник
	);
...
//эмуляция перечисления РезультатСравнения
&НаКлиентеНаСервереБезКонтекста
...
//эмуляция перечисления ТипыИсточникаПравил
&НаКлиентеНаСервереБезКонтекста
Функция ТипыИсточникаПравил()
	Возврат Новый ФиксированнаяСтруктура("Файл, Макет", 0, 1);
КонецФункции

//эмуляция перечисления ТипыОбъектовОбмена
&НаКлиентеНаСервереБезКонтекста
...
//эмуляция перечисления МетаданыеТиповОбъектовОбмена
&НаСервереБезКонтекста
...
//возвращает соответствие содержащие перечень типов объектов 
//(Справочник, Документ и т.д.)
//вошедших в состав плана обмена с указанием запрещена
//авторегистрация по данному объекту -- Истина
//или нет -- Ложь
&НаСервереБезКонтекста
Функция ТипыОбъектовВошедшиеВСоставПланаОбмена()
	...
	Для Каждого элт Из планОбмена.Состав Цикл
		...
		Иначе
			//если нет ни обного объекта заданного типа с запретом авторегистрации
			//указываем, что все объекты данного типа используют авторегистрацию
...
Процедура ПроверитьСоответствиеНастроекСоставаПланаОбменаИПодписокРегистрацииИзменений()
	...
	мНесоответствияПО = Новый Массив; //несоответствия по плану обмена (ПО)
	...
	мНесоответствияПС = Новый Массив; //несоответствия по подпискам на событие (ПС)
...
Процедура ПроверитьРегистрацию_СправочникПрочиеДоходыИРасходыСервер()
	...
	Пока СписокОбъектов.Следующий() Цикл
		...
		Если дмнОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() = Истина Тогда //в ПБ ничего не регистрируем
			Если получатели.Количество()>0 Тогда
				Для Каждого узел Из получатели Цикл
					ошибки.Добавить("Ошибка регистрации:"+текСсылка+"для ПБ запрещена регистрация на узле:"+узел);
				КонецЦикла;
			КонецЕсли;
		Иначе			
...
&НаСервереБезКонтекста
Процедура ПроверитьРегистрацию_СправочникяСхВидыУдобренийИСЗРСервер()
	ошибки = Новый Массив;
	СписокОбъектов = Справочники.яСхВидыУдобренийИСЗР.Выбрать();
	мУзлы = ПолучитьМассивУзловПланаОбмена();
	имяПланаОбмена = ИмяПланаОбмена();
...
&НаКлиенте
Процедура ТестДолжен_ПроверитьРегистрацию_СправочникяСхВидыУдобренийИСЗР() Экспорт
	ПроверитьРегистрацию_СправочникяСхВидыУдобренийИСЗРСервер();
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроверитьРегистрацию_СправочникПодразделенияОрганизацийСервер()
	ошибки = Новый Массив;
	...
	запросУзловПоОрганизации = Новый Запрос("ВЫБРАТЬ
	                                        |	дмнБСХП_БСХП.Ссылка КАК Ссылка
	                                        |ИЗ
	                                        |	ПланОбмена.дмнБСХП_БСХП КАК дмнБСХП_БСХП
	                                        |ГДЕ
	                                        |	дмнБСХП_БСХП.ЭтотУзел = ЛОЖЬ
	                                        |	И дмнБСХП_БСХП.Организация = &Организация");
	
	СписокОбъектов = запрос.Выполнить().Выбрать();
	мУзлы = ПолучитьМассивУзловПланаОбмена();
	имяПланаОбмена = ИмяПланаОбмена();
	...
	Пока СписокОбъектов.Следующий() Цикл
		...
		Если дмнОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() = Истина Тогда //в ПБ ничего не регистрируем
			...
			КонецЕсли;
		Иначе	
			//регистрируем объект только в узлах с соответствующей организацией
			запросУзловПоОрганизации.УстановитьПараметр("Организация", СписокОбъектов.Организация);
			выб = запросУзловПоОрганизации.Выполнить().Выбрать();
			количествоУзловПоОрганизации = выб.Количество();
			Если количествоУзловПоОрганизации <> получатели.Количество() Тогда
				ошибки.Добавить("Ошибка регистрации:"+текСсылка+"для организации:"+СписокОбъектов.Организация
								+" зарегистрирован на :"+получатели.Количество()+" узлах"
								+" узлов для организации:"+количествоУзловПоОрганизации
				);
			КонецЕсли;
				
			Для Каждого узел Из получатели Цикл
				Если СписокОбъектов.Организация <> узел.Организация Тогда
					ошибки.Добавить("Ошибка регистрации:"+текСсылка+"для организации:"+СписокОбъектов.Организация
									+" зарегистрирован на узле:"+узел
									+"для организации:"+узел.Организация
					);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
&НаКлиенте
Процедура ТестДолжен_ПроверитьРегистрацию_СправочникПодразделенияОрганизаций() Экспорт
	ПроверитьРегистрацию_СправочникПодразделенияОрганизацийСервер();
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроверитьРегистрацию_СправочниксхСтруктураПосевныхПлощадейСервер()
	ошибки = Новый Массив;
	
	запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1000
	                      |	схСтруктураПосевныхПлощадей.Ссылка КАК Ссылка,
	                      |	схСтруктураПосевныхПлощадей.Организация КАК Организация
	                      |ИЗ
	                      |	Справочник.схСтруктураПосевныхПлощадей КАК схСтруктураПосевныхПлощадей"
	);
...
&НаКлиенте
Процедура ТестДолжен_ПроверитьРегистрацию_СправочниксхСтруктураПосевныхПлощадей() Экспорт
	ПроверитьРегистрацию_СправочниксхСтруктураПосевныхПлощадейСервер();
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроверитьРегистрацию_РегистрСведенийдмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассуСервер()
	ошибки = Новый Массив;
	
	запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1000
	                      |	дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	                      |	дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу.Номенклатура КАК Номенклатура
	                      |ИЗ
	                      |	РегистрСведений.дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу КАК дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу.НоменклатурнаяГруппа,
	                      |	дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу.Номенклатура"
	);
	...
	Пока СписокОбъектов.Следующий() Цикл
		источник = РегистрыСведений.дмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу.СоздатьНаборЗаписей();
		источник.Отбор.НоменклатурнаяГруппа.Установить(СписокОбъектов.НоменклатурнаяГруппа);
		источник.Отбор.Номенклатура.Установить(СписокОбъектов.Номенклатура);
		источник.Прочитать();
		
		отказ = Ложь;
		//вылавливаем ошибки в работе типового механизма регистрации изменений
		Попытка
			ОбменДаннымиСобытия.МеханизмРегистрацииОбъектовПередЗаписьюРегистра(ИмяПланаОбмена(), источник, Отказ, Истина);
		Исключение
			ошибка = ОписаниеОшибки();
			ошибки.Добавить("Ошибка регистрации набора записей для номенклатурной группы:"+СписокОбъектов.НоменклатурнаяГруппа
							+" номенклатура: "+СписокОбъектов.Номенклатура
							+" "+ошибка
			);
		КонецПопытки;
		...
		Если дмнОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() = Истина Тогда //в ПБ ничего не регистрируем
			Если получатели.Количество()>0 Тогда
				Для Каждого узел Из получатели Цикл
					ошибки.Добавить("Ошибка регистрации набора записей для номенклатурной группы:"+СписокОбъектов.НоменклатурнаяГруппа
									+" номенклатура: "+СписокОбъектов.Номенклатура
									+"для ПБ запрещена регистрация на узле:"+узел);
				КонецЦикла;
			КонецЕсли;
		Иначе						
			//в получателях должны быть все узлы обмена, кроме текущего
			Для Каждого узел Из мУзлы Цикл
				Если НЕ получатели.Содержит(узел) Тогда
					ошибки.Добавить("Ошибка регистрации набора записей для номенклатурной группы:"+СписокОбъектов.НоменклатурнаяГруппа
									+" номенклатура: "+СписокОбъектов.Номенклатура
									+"не зарегистрирован на узле:"+узел);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ТестДолжен_ПроверитьРегистрацию_РегистрСведенийдмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассу() Экспорт
	ПроверитьРегистрацию_РегистрСведенийдмнКоэффициентыПересчетаПродукцииРастенееводстваВЗеленуюМассуСервер();
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроверитьРегистрацию_РегистрСведенийдмнПроцентРаспределенияРасходовПоНоменклатурнымГруппамСервер()
	
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_ПроверитьРегистрацию_РегистрСведенийдмнПроцентРаспределенияРасходовПоНоменклатурнымГруппам() Экспорт
	ПроверитьРегистрацию_РегистрСведенийдмнПроцентРаспределенияРасходовПоНоменклатурнымГруппамСервер();
КонецПроцедуры
